...
def prepare_application(session: Any, *, timeout: float = 20.0,
    ...
    with session.journal.call("prepare_application"):
        prepared = document.prepare_document(main, timeout=timeout, apply_hint=apply_hint)
...
def locate_application(session: Any, url: str, *, timeout: float = 25.0,
    ...
    *plus* the candidate's own state wait below — the workflow from there is the
    ...
    # Caller-supplied candidates (a learned route from a skill, or the collector's own
    # rule lookup) lead route-first; the built-in ATS rules are the fallback.
    ...
    with session.journal.bind(stage="navigate"):
        try:
            ...
            if landing == url:
                raise
            # A candidate route is a guess (a learned URL may carry an expired session
            # token); the posting is not. Measured 2026-08-29: 11 of 12 forms lost to a
            # learned-route cache died right here — timeout, unresponsive renderer or a
            # navigation error on the cached URL — with the posting never visited.
    ...
    for _hop in range(hop_budget):
        ...
        with session.journal.bind(stage="inspect", hop=hop):
            ...
            pending_state = None
            prepared = prepare_application(session, timeout=timeout, apply_hint=apply_hint)
        ...
        row = {
            ...
            "apply_link_label": prepared.get("apply_link_label"),
            "apply_link_selector": prepared.get("apply_link_selector"),
...
def run_application(session: Any, url: str, *,
                    ...
                    apply_hint: dict[str, Any] | None = None,
    ...
    located = locate_application(session, 
        ...
        hop_budget=hop_budget, candidates=candidates, apply_hint=apply_hint)
