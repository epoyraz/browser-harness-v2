...
#: The factual half of the page read, as a composable fragment.
#:
#: It defines `fields`, `files`, `groups`, `submits`, `inDom`, `visible`, `why` and
#: `textLen` and deliberately does not return: a caller appends its own verdict and
#: closing, so extracting the DOM once and judging it twice never costs two round trips.
#: `applications/document.py` is the other caller — the classification that used to live
...
#: Core's own ending: generic form identity, and nothing about what kind.
_SCHEMA_VERDICT_JS = """
  // Generic form identity: are there real fillable controls here, and can they be
  // submitted. What KIND of form it is — an application, a login, a newsletter box — is
  // domain judgment and is composed on by whoever holds that knowledge.
  const isForm = fields.length >= 2 && submits.length > 0 || files.length > 0;
  let reason;
  if (isForm) reason = fields.length + ' field(s) with a submit control';
...
_SCHEMA_JS = _SCHEMA_BODY_JS + _SCHEMA_VERDICT_JS
