...
| | v1 | v2 |
|---|---:|---:|
| Runtime lines | 5,900 | 11,016 core |
| Dependencies | 4 | **1** |
...
| Result contract | dicts | typed `Outcome` / `Class` |

**4.1× the tests for 1.9× the code**, so test density per line is over twice v1's. v1 has
no agent-facing concurrency at all — its `http_get` docstring says *"Wrap in
ThreadPoolExecutor for bulk"*, which hands the problem back to the caller. Its
`telemetry.py` is product analytics; there is no run journal to ask what a run did.
...
isolated world where the page can neither see nor break it; failures carry a typed class
and a recovery line; `bh --doctor` classifies why a browser can or cannot be reached across
...
**It is nearly twice the code.** Part of that is owning the stack — v2 depends on
`websockets` alone where v1 pulls `cdp-use`, `fetch-use` and Pillow — but not all of it.
The remaining size is capability, and capability you do not use is cost you do pay.
...
```
harness/       11,016   browser primitives; no domain knowledge
